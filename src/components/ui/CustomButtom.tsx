...
export default function CustomButton({ color = "primary", className, children, ...props }: CustomButtonProps) {
  const themeClasses = useMemo(() => {
    switch (color) {
      ...
      default:
        return "bg-linear-120 from-default/75 via-default to-default/75 text-default-foreground hover:from-default-foreground/75 hover:via-default-foreground hover:to-default-foreground/75 hover:text-default";
    }
  }, [color]);
