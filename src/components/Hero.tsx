...
export default function Hero({
  ...
  React.useEffect(() => {
    ...
    const spans = letters.map((letter, i) => (
      <span
        key={i}
        className={`opacity-0 duration-1000 ${i === letters.length - 1 && "!tracking-normal"}`}
      >
        {letter}
      </span>
    ));
  ...
  React.useEffect(() => {
    ...
    const updateFontSize = () => {
      ...
      const subtitleSize = (titleSize / 7) * 4;
  ...
  function rotateSubtitle(rotated: boolean) {
    ...
    const subtitle = subTitleRef.current;
    if (rotated) subtitle.classList.add("-scale-x-100");
    else subtitle.classList.remove("-scale-x-100");
  }
          ...
          <div className="relative flex justify-center items-center">
            ...
            <div className="absolute inset-0 flex flex-col justify-evenly items-center">
              ...
              >
                <h1
                  ref={titleRef}
                  className={`pb-1 h-full text-white drop-shadow-[0_2px_8px_rgba(255,255,255,0.5)] ${anta.className}`}
                >
                  {lettersSpan}
                </h1>
              ...
              <div
                ref={sloganRef}
                className={`flex flex-col justify-between items-center opacity-0 duration-1000 ${kode_mono.className}`}
              >
