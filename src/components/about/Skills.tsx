...
export default function Skills() {
  return (
    <div className="w-full h-full flex flex-col justify-between gap-3">
      <AboutSection
        title="Langages"
        images={[java, c, nodeJs, typescript, python, ruby]}
        wait={1000}
      />
      <AboutSection
        title="Frameworks"
        images={[spring, springboot, dotnet, nextjs, react, angular]}
        wait={2700}
      />
      <AboutSection
        title="Bases de données"
        images={[sql, mysql, postgresql, mssql, plsql]}
        wait={4400}
      />
      <AboutSection
        title="Technologies web"
        images={[html, css, javascript, bootstrap, tailwind]}
        wait={5900}
      />
    </div>
  );
