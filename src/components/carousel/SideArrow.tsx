...
export default function SideArrow({
  ...
  React.useEffect(() => {
    if (disabled)
      setClassList(
        `${reversed ? "translate-x-full" : "-translate-x-full"} opacity-0`,
      );
  ...
  return (
    <div
      ref={containerRef}
      className={`w-16 h-full flex flex-col justify-center ${reversed ? "items-end" : "items-start"}`}
    >
      <svg
        ref={arrowRef}
        onClick={disabled ? undefined : () => action()}
        className={`duration-500 ${classList} ${reversed ? "rotate-90" : "-rotate-90"}`}
