...
export default function DownArrow({ action, text, disabled }: { action: Function, text: string, disabled: boolean }) {
    ...
    React.useEffect(() => {
        if (disabled) 
            setClassList("-translate-y-full opacity-0")
        else 
    ...
    return (
        <div onClick={disabled ? undefined : () => action()} className={`py-5 flex flex-col items-center duration-500 ${classList}`}>
            <svg className="h-10" ref={arrowRef} xmlns="http://www.w3.org/2000/svg" viewBox="0 100 500 300">
