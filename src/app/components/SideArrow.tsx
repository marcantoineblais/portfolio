export default function SideArrow({ action, reversed }: { action: Function, reversed: boolean }) {
  ...
  return (
    <div onClick={() => action()} className="w-1/12 flex flex-col items-center cursor-pointer hover:opacity-75">
      <svg className={`h-full ${reversed ? "rotate-90" : "-rotate-90"}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500">
