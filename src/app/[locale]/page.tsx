...
export default function Home() {
  ...
  const components = useMemo(
    () => [hero, catcam, scrabble, skills, technology, contact],
    [hero, catcam, scrabble, skills, technology, contact],
  );
  ...
  React.useEffect(() => {
    const calculateScrollHeight = () => {
      ...
      components.forEach((component, i) => {
        if (
          ...
          i > 0 &&
          component.start > currentHeight &&
          components[i - 1].end < currentHeight
        ) {
          // scrolling to
          component.setOpacity(
            (currentHeight - components[i - 1].end) /
              (component.start - components[i - 1].end),
          );
          component.setIsRendered(true);
        } else if (
          i < components.length - 1 &&
          component.end < currentHeight &&
          components[i + 1].start > currentHeight
        ) {
          // scrolling from
          component.setOpacity(
            (components[i + 1].start - currentHeight) /
              (components[i + 1].start - component.end),
          );
          component.setIsRendered(true);
        } else {
          // out of view
      ...
      timers.scrollTimeout = window.setTimeout(
    ...
    const scrollToNearestSection = () => {
      ...
      components.forEach((component) => {
        if (
          Math.abs(currentHeight - component.center) <
          Math.abs(currentHeight - nearestSection.center)
        )
  ...
  function renderMainComponents() {
    return components.map(({ component, isRendered }) =>
      isRendered ? component : null,
    );
  }
